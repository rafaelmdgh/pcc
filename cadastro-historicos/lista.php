...
*/
$stmt = $pdo->prepare("SELECT * FROM historico INNER JOIN tipo_historico ON tipo_codigo = historico_tipo WHERE historico_usuario = ".$_SESSION["usuario_codigo"]."  ORDER BY historico_codigo ASC LIMIT ".$limite." OFFSET ".$offset.";");
...
<input type="button" class="btn btn-primary" onclick="location.href='cadastrar.php'" value="Adicionar" />
    ...
    $sqlTotal = 'SELECT COUNT(*) total FROM historico WHERE historico_usuario = '.$_SESSION['usuario_codigo'];
